...
온통청년 API에서 정책 2,631건을 모아 정제하고, 임베딩 → 벡터검색 → 선택적 리랭킹 → 답변 생성으로 이어지는 RAG 파이프라인을 직접 구축했습니다. LLM에 검색 결과를 그냥 던지는 대신 단계를 쪼갰고, 검색이 실제로 맞는지 평가셋으로 재본 다음 리랭킹의 효과와 회귀를 함께 측정했습니다. 현재 min instances 0인 Cloud Run 공개 경로는 CPU·메모리 조건 때문에 리랭커를 끈 `RERANK=0`을 사용합니다.
...
백엔드 주력은 Spring Boot(Java 17)로 두고, 임베딩·리랭킹 같은 ML 부분만 Python으로 떼어낸 마이크로서비스 구조입니다. 임베딩과 리랭킹은 외부 모델 API 대신 컨테이너의 로컬 모델을 사용하므로 모델 API 호출 한도가 없습니다. 다만 Cloud Run 실행 자원과 Gemini 답변 생성에는 각 서비스의 비용·할당량 조건이 적용됩니다.

## 검색 품질

질문 60개짜리 평가셋을 만들어(각 질문에 정답 정책을 라벨링) 정답이 상위에 오는지 측정했습니다. 이 평가에서 리랭커(bge-reranker-v2-m3)를 붙였더니 1순위 정답률(recall@1)이 **40%에서 52%로** 올랐습니다.
...
recall@5와 recall@10은 각각 60문항 기준 1문항만큼 감소했습니다. 표본이 작아 통계적 유의성을 판단하지 않았으며, 1순위 개선과 함께 이 회귀도 그대로 기록합니다. 평가셋 생성은 `eval/make_evalset.py`, 측정은 `eval/run_eval.py`(기본)와 `eval/run_eval_rerank.py`(리랭킹)로 재현할 수 있습니다.
...
| 리랭커 | `BAAI/bge-reranker-v2-m3` (평가·로컬 선택 경로, 현재 Cloud Run은 `RERANK=0`) |
...
- **RAG를 단계별로 직접 구성했습니다.** 검색 결과를 LLM에 통째로 넘기지 않고 임베딩·벡터검색·선택적 리랭킹·생성을 나눠 두니, 어느 단계가 품질을 좌우하는지 평가셋으로 따로 재볼 수 있었습니다. 공개 배포는 리랭킹 평가 결과와 별개로 `RERANK=0` 경로를 사용합니다.
- **임베딩은 로컬 모델로 돌립니다.** 처음엔 Gemini 임베딩을 쓰려 했는데 무료 티어가 분당·일일 한도에 금방 걸려서, 한국어에 강한 e5 모델을 로컬로 돌리는 쪽으로 바꿨습니다. 외부 임베딩 API의 호출 비용과 한도는 제거했지만 실행 인프라 비용은 별도입니다.
...
- **지금은 청년정책만 다룹니다.** 전국민 대상 gov24 적재는 과거 odcloud 인증 문제로 보류했습니다. 2026-07-22 [공식 데이터 페이지](https://www.data.go.kr/data/15113968/openapi.do)에는 v3 API가 무료·자동승인·개발계정 10,000회로 안내되지만, 현재 저장소에는 인증키와 샘플 응답이 없어 실제 권한·응답 스키마를 재검증하지 않았습니다. 공식 소량 응답을 확인하기 전에는 파서를 추측해 추가하지 않습니다.
...
로컬·GitHub·Pages·Cloud Run의 2026-07-22 재검증 상태와 현재 공개 artifact는
[정본·공개 상태 재검증](docs/operations/STATE_REVALIDATION_2026-07-22.md)에 구분해 기록했습니다.
