...
- **지역 필터는 넣지 못했습니다.** 공공데이터의 지역코드가 부정확하고(지자체 정책에 엉뚱한 지역 코드가 섞여 있기도 합니다) 기관명도 부서명만 들어있는 경우가 많아, 믿을 만한 지역 필터를 만들기 어려웠습니다. 그래서 지역 대신 지원 내용으로 검색하는 방식으로 정리했고, 지역 필터는 데이터를 정제한 뒤의 과제로 남겨뒀습니다.
