...
- Gains:
  - `c2d-025` 세종 청년센터: baseline rank 0 (in_top30 rank 14 but filtered by cosine, score 0.868) → candidate rank 3. `lex_b ['시에서','청년이','청년센터를',…]` overlap 0 → `lex_c ['청년','청년센터',…]` overlap 2. Vector-only rank 6 → lexical rescued a threshold-filtered case.
...
| c2d-025 세종 청년센터 | culture | 0/14 filtered | 3/3 | 6/6 | **yes**: baseline in top30 (14) but cosine filtered (score 0.868 < top competitors but >0.78 yet filtered? Actually score 0.868 >0.78 but rank 0 means not in bi top10? Wait artifact shows filtered_by_cosine true because gold in top30 but not in bi top10 — indicates gold was beyond cosine threshold ordering? Need check: baseline top30 rank 14, but bi rank 0 means threshold removed it from top10? Actually bi is top10 after threshold; gold was beyond 10 even after bias. Candidate brought to 3, so lexical bias overcame threshold/rank. | **lexical**: fixing `시에서` noise and particle strip moved rank 14→3 |
...
1. **Threshold/top30**: Only 1 of 8 misses (c2d-025) was threshold/top10 induced (in top30 but outside top5/10); candidate fixed it. Other 6 persistent misses are **not in top30 at all**, so COSINE_MIN and top10 cutoff are innocent — root cause is **vector retrieval** not returning gold within 30, regardless of lexical. Only `c2d-031` was in top30 (7) and lexical moved it.
