...
def main():
    ...
    # clean tree check per spec section 9
    try:
        dirty_at_start = bool(subprocess.check_output(["git", "status", "--porcelain"], cwd=str(ROOT), stderr=subprocess.DEVNULL).decode().strip())
    except Exception:
    ...
    try:
        evaluator_commit = subprocess.check_output(["git", "log", "-1", "--format=%H", "--", str(ROOT / "eval" / "retrieval_v2" / "run_dev_baseline.py")], cwd=str(ROOT), stderr=subprocess.DEVNULL).decode().strip()
    except Exception:
