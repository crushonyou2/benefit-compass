...
- **Incident — holdout plaintext audit seal repair (aggregate-only, Exp3 pre-retrieval, no retrieval/DB/model):** post-tuning candidate session에서 `eval/test_retrieval_v2_cycle2_devset.py::_load_holdout_items()`가 고정 ref `git show`로 holdout plaintext를 process 메모리에 읽음 — `stdout`/agent-visible case content·retrieval·rank·score 노출 0, Exp3 retrieval 미실행. HARD SEAL 충돌로 판단하고 `RETRIEVAL_V2_ALLOW_HOLDOUT_PLAINTEXT_AUDIT=1` opt-in 없이는 `_load_holdout_items()`가 `git show` 호출 전 `unittest.SkipTest`로 fail-safe 차단되도록 repair. default candidate-session은 holdout ref를 절대 읽지 않음; opt-in에서만 기존 fail-closed audit(40건 집계, UTF-8/JSON/format 실패 시 AssertionError) 유지. 대상: cycle1/cycle2 holdout query/gold overlap 4건 + helper 1건이 default에서 skipped, dev 무결성 나머지는 pass. 본 incident는 seal 완화가 아니며 strict seal 최종 판정은 final-gate 전 Web/user 판단 필요. Exp3 untracked 2파일은 본 commit 미포함, `ml-service`/frozen dev/holdout bytes 미변경, D-009 불변.
## Next state
