...
| Q-005 | Retrieval v2 after Cycle2 closure — Cycle2가 Exp4 REJECTED(30/36 vs 30, D-010 bounded, PROCESS AUDIT 2026-08-30 durable-only + Web cross-validation addendum)로 후보 없이 종료된 뒤, 별도 Cycle3 evaluation/candidate-search cycle을 새로 시작할지? (Web recording으로 Exp1 post-result extra retrieval, Exp2 2회 premature+steering, Exp3/Exp4 premature+steering이 확정되어 Cycle2 전체 `PROCESS_CONTAMINATED` — 28/30/30 수치는 `VALID_CANONICAL_RESULT`이나 완전성 결론은 무효. D-010상 dev 36 SHA `c8b66fef…`는 tuning set 유지 가능하나, 보수적 대안으로 `fresh Cycle3 dev + fresh holdout` 병행 생성을 권고. 최종 선정은 반드시 fresh holdout(보수적 대안에서는 fresh dev 포함)에서만 가능 — `single execution`은 `canonical accepted 1회`로 정정. D-010은 불변, 선택은 본 Q-005에서 결정) | 2026-08-30 | closed → D-011 |

| Q-006 | Retrieval v3 — exact final dev/holdout sizes, confidence/precision rule, user-centered paired latency budget and final pilot protocol remain unresolved; to be fixed after retrieval-blind pilot and before candidate tuning / one-way protected evaluation — 2026-09-01 open | 2026-09-01 | closed → D-014 |
